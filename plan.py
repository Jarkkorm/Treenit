...
def get_list():
	...
	sql = "SELECT P.id, P.name, P.duration FROM plan P, users U " \
		"WHERE P.user_id=U.id AND U.id=:user_id AND P.visible = 1"
...
def new_plan(name, duration):
    ...
    if user_id == 0:
        return -1
    sql = "INSERT INTO plan (name, duration, user_id, visible) VALUES (:name, :duration, :user_id, 1) RETURNING id"
...
def delete_plan(id):
    sql = "UPDATE plan SET visible=0 WHERE id=:id"
    result = db.session.execute(sql, {"id":id})
    return result